...
const bcrypt=require("bcryptjs");//encryption decryption
const helmet=require("helmet");//to add secure header into response
const compression=require("compression");//to compress file which is going to deliver to client browser into less space
// const morgan=require("morgan");//this package is used to print request data
...
const fs=require("fs");
// const https=require("https");//for sending ssl certificate to the client
...
app.use(helmet());//to add secure header into response 
app.use(compression());//to compress file which is going to deliver to client browser into less space
// app.use(morgan("combined",{stream:accessLogStream}));//this package is used to print request data
 
app.use(bodyParser.urlencoded({extended:false}));//for extracting data of form

// const privateKey=fs.readFileSync("server.key");//for sending ssl certificate to the client
// const certificate=fs.readFileSync("server.cert");//for sending ssl certificate to the client
...
mongoose.connect("mongodb+srv://urvish:urvish@ecom.mehbxhu.mongodb.net/ecom?retryWrites=true&w=majority")
.then((result)=>{
    ...
    // https.createServer({key:privateKey,cert:certificate},app)//for sending ssl certificate to the client
    // .listen(DB_PORT,DB_HOST,()=>{
    //         console.log("Server is created at port 3000...");
    //     });
})    
.catch((err)=>{
